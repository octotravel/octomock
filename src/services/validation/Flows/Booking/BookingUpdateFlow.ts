...
export class BookingUpdateFlow extends BaseFlow implements Flow {
  ...
  private validateBookingUpdateUnitItems = async (): Promise<BookingUpdateUnitItemsScenario> => {
    const [bookableProduct] = this.config.productConfig.availableProducts;
  ...
  private validateBookingUpdateContact = async (): Promise<BookingUpdateContactScenario> => {
    const [bookableProduct] = this.config.productConfig.availableProducts;
  ...
  private validateBookingUpdateProduct = async (): Promise<BookingUpdateProductScenario> => {
    const [bookableProduct1, bookableProduct2] = this.config.productConfig.availableProducts;
