...
export class OctoFlowValidationService {
  ...
  public validateFlow = async (): Promise<void> => {
    ...
    const suppliersErrors = suppliers
      .map((supplier) => {
    ...
    const supplier = (
      await this.api.getSupplier({ id: suppliers[0].id }, params)
    ).result;
    ...
    const product = (await this.api.getProduct({ id: products[0].id }, params))
      .result;
    ...
    const availabilities = (
      await this.api.getAvailability(
        {
    ...
    const availabilityErrors = availabilities
      .map((availability) => {
    ...
    const bookingCreate = (
      await this.api.bookingReservation(
        {
          productId: product.id,
          optionId: product.options[0].id,
    ...
    const bookingConfirm = (
      await this.api.bookingConfirmation(
        {
          uuid: bookingCreate.uuid,
          unitItems: [
            { unitId: product.options[0].units[0].id },
            { unitId: product.options[0].units[0].id },
          ],
    ...
    const bookingCancel = (
      await this.api.cancelBooking({ uuid: bookingConfirm.uuid }, params)
    ).result;
